import { precacheAndRoute } from 'workbox-precaching';
import { registerRoute, setCatchHandler } from 'workbox-routing';
import { NetworkFirst, CacheFirst } from 'workbox-strategies';

//precache assets generated by Vite/Workbox
precacheAndRoute(self.__WB_MANIFEST);

//cache images/icons only at runtime, JS/CSS are already handled by precache manifest
//using CacheFirst for chunks can serve stale files after a new deploy
registerRoute(
    ({ request }) => request.destination === 'image',
    new CacheFirst({ cacheName: 'assets' }),    //cache first
);

//for page navigations, try network first so users get fresh data
//if network is slow or unavailable, fall back to cached pages after 3s
registerRoute(
    ({ request }) => request.mode === 'navigate',
    new NetworkFirst({
        cacheName: 'pages',
        networkTimeoutSeconds: 3,
    }),
);

//cache offline.html during SW install
self.addEventListener('install', (event) => {
    event.waitUntil(
        (async () => {
            const cache = await caches.open('offline');
            await cache.add('/offline.html');
        })(),
    );
    self.skipWaiting(); //new SW skips waiting
});

//ensures sw controls the page on activation without a reload
self.addEventListener('activate', (event) => {
    event.waitUntil(self.clients.claim());
});

//fallback to offline.html if navigation fails
setCatchHandler(async ({ event }) => {
    if (event.request.mode === 'navigate') {
        const cache = await caches.open('offline');
        const cached = await cache.match('/offline.html');
        return cached || Response.error();
    }
    return Response.error();
});

//push notification handler
self.addEventListener('push', (event) => {
    const data = (event.data && event.data.json && event.data.json()) || {};
    const title = data.title || 'Expense Tracker';
    const body = data.body || 'Notification';
    const url = data.url || '/app/dashboard';

    event.waitUntil(
        self.registration.showNotification(title, {
            body,
            data: { url },
        }),
    );
});

//open/focus app on notification click
self.addEventListener('notificationclick', (event) => {
    console.log('notificationclick', event.notification.data);
    event.notification.close();
    const url = (event.notification.data && event.notification.data.url) || '/';

    event.waitUntil(
        (async () => {
            //get all window clients
            const allClients = await clients.matchAll({ 
                type: 'window', 
                includeUncontrolled: true,
            });

            for (const client of allClients) {
                if('focus' in client) {
                    await client.focus();
                    //navigate existing tab to the target URL
                    if('navigate' in client) {
                        return client.navigate(url);
                    }
                    return;
                }
            }

            //else open new window/tab
            return self.clients.openWindow(url);
        })(),
    );
});
